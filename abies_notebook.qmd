...
Les données collectées par Wood et son équipe [@wood_data_2016] sont disponibles sous licence [CC0 1.0](https://creativecommons.org/publicdomain/zero/1.0/deed.fr) depuis le site [Dryad](https://datadryad.org/stash/dataset/doi:10.5061/dryad.r3645). Le protocole et les métadonnées sont explicités dans la publication correspondante [@wood_2015].
...
[] -   Les hauteurs et diamètres ont des distributions asymétriques qui s'étalent à gauche.
[] -   Les hauteurs et diamètres ont des distributions asymétriques qui s'étalent à droite.

[] -   Le nuage de points est parfaitement linéaire.
[] -   Le nuage de points est linéaire, mais s'infléchit pour les valeurs élevées.
...
Nous partirons de ces dernières données du logarithme de la hauteur en fonction du diamètre qui montre le meilleur nuage de point considérant que l'homoscédasticité est également souhaitable pour une régression non linéaire par les moindres carrés. Par facilité, nous calculons la variable `logheight` comme étant le logarithme népérien de `height`.
...
<!--% Réalisez un modèle de von Bertalanffy de logheight en fonction de diameter. Ne forcez pas le modèle à passer par 0. Calculez le résumé du modèle et formatez-le avec tabularise(). Tracez enfin le graphique du modèle ajusté dans les données. -->
...
<!--- Comparez les trois modèles réalisés à l'aide du critère d'Akaike. -->
